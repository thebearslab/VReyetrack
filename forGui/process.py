...
class Process():
    ...
    def check_point_on_sphere(self, cx, cy, cz, point, r):
    ...
    # switch y and z axis
    def unity_to_python_point(self, old):
    ...
    def rotate_vec_by_quaternion(self,quat, vec):
        quatx = quat[0]
        quaty = quat[1]
        quatz = quat[2]
        quatw = quat[3]
    ...
    def get_hit_point_draw(self, combined_ray, linecolor):
        start = np.array([0.0, 0.0, 0.0])

        rot = np.array([0.0, 1.0, 0, 0.0])
        # rotate a vector by quaternion rotation
        dir = Process.rotate_vec_by_quaternion(rot, combined_ray)
        end = start + dir
        ...
        Process.unity_to_python_point(start)
        Process.unity_to_python_point(end)
        Process.unity_to_python_point(hitpoint)
    ...
    def transform_to_equirectangular(self, point, geo_w, geo_h, color):
    ...
    def writeCSV(self):
        ...
        print ("the folder has the name %s" % (self.dirname))

        center = Process.getHittingPoint()

        tobii_folder_name = 'XYtable'
        tobii_path = ''
        participants_files = os.listdir(self.dirname)
        for par_id in participants_files:
            # files are 001, 002
            if len(par_id) == 3:
                single_par_folder = os.listdir(self.dirname + '/' + par_id)
                for single_file in single_par_folder:
                    if tobii_folder_name in single_file:
                        tobii_path =self.dirname  + '/' + par_id + '/' + single_file
                                ...
                                for child in root:
                                    ...
                                    for c in child:
                                        ...
                                        if 'CombinedGazeRayWorld' in c.tag:
                                            ...
                                            hitpoint_2d = Process.get_hit_point_draw(combined_ray, 'g')
                                            projPoint_w = Process.transform_to_equirectangular(hitpoint_2d, 3840, 1080, 'g')
